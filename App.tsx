          ...
          {(userRole !== 'FISCAL') && (
            <div className="mt-4 px-2">
                ...
                {(userRole === 'LIDER' || userRole === 'LIDER_REGIONAL' || userRole === 'USUARIO') && (
                    ...
                    <SidebarItem indent page="interview-list" label="Bandeja de Entrevistas" icon={<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>} />
             ...
             <div className="flex items-center gap-4 border-l border-slate-200 pl-6">
                 <div className="text-right hidden md:block">
                   <div className="text-xs font-black text-slate-800 uppercase tracking-tight">
                     {userRole === 'FISCAL' ? 'Unidad de Conocimiento' : userRole === 'GESTOR' ? 'Oficina de Radicación' : userRole === 'LIDER' ? 'Coordinación Nacional' : userRole === 'LIDER_REGIONAL' ? 'Coordinación Regional' : 'Célula de Evaluación'}
                   </div>
                   <div className="text-[9px] text-blue-600 font-black uppercase tracking-widest">
                     {userRole === 'FISCAL' ? 'Protección' : userRole === 'GESTOR' ? 'Correspondencia' : 'Evaluación Técnica'}
                   </div>
                 </div>
                 <div className={`h-10 w-10 rounded-xl flex items-center justify-center font-black text-white shadow-md ${userRole === 'FISCAL' ? 'bg-blue-600' : userRole === 'GESTOR' ? 'bg-slate-800' : userRole === 'LIDER' ? 'bg-indigo-600' : userRole === 'LIDER_REGIONAL' ? 'bg-orange-600' : 'bg-emerald-600'}`}>
                   {userRole === 'FISCAL' ? 'F' : userRole === 'GESTOR' ? 'GD' : userRole === 'LIDER' ? 'L' : userRole === 'LIDER_REGIONAL' ? 'LR' : 'U'}
                 </div>
             </div>
          </div>
        </header>
